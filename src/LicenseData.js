...
class LicenseData extends Component {
    ...
    async loadBlockchainData() {
      ...
      const carPark = new web3.eth.Contract(CAR_PARK_LICENSE_ABI, CAR_PARK_LICENSE_ADDRESS)
      this.setState({ carPark })
      const surName = await carPark.methods.IDtoSurName(this.props.idLicense).call();
      this.setState({surName});
      const firstName = await carPark.methods.IDtoFirstName(this.props.idLicense).call();
      this.setState({firstName});
      const birthDate = await carPark.methods.IDtoBirthDate(this.props.idLicense).call();
      this.setState({birthDate});
      const birthPlace = await carPark.methods.IDtoBirthPlace(this.props.idLicense).call();
      this.setState({birthPlace});
      const vehicleNumber = await carPark.methods.IDtoVehicleNumber(this.props.idLicense).call();
      this.setState({vehicleNumber});
    }
